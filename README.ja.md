...
SSH Fleet は MCP 経由でリモートワークスペースを操作し、個別の権限付与と複数の独立した PTY を使用します。Lattice Remote の単一ターミナル共有だけでは、リモートの複数エージェントを編成できません。Relay 経由の Fleet は未完成です。ホスティング型のチームサービスは現在提供していません。
