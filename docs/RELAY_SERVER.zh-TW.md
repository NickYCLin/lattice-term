...
- **被分享端**：「分享這台裝置」→ 分享方式選「透過中繼伺服器」，
  ...
  配對碼；可設定 6～64 個字元的固定密碼（大小寫英文、數字、半形特殊符號），或每次分享自動產生長碼。只有中繼固定密碼可明確選擇保存，區網直連與隨機碼不會保存。
...
- 連線成功的裝置會留在「我的連線」，之後從清單連線只需要輸入配對碼，
  不必再重打九位數與中繼位址。檢視端可明確選擇把配對碼放進安全認證後端；
  不選擇保存時仍只供當次使用。
...
- **無人值守（固定配對碼）**：桌面內嵌分享只允許保存中繼模式的固定密碼，
  而且由原生層綁定永久裝置 ID；明文不進 WebView settings、程序參數或日誌，
  只以 stdin 交給 Agent。`credential_backend.json` 以權威位置標記與
  `remoteHostCleanupPending` 持久清理日誌記錄狀態；主機只載入標記指向的後端，
  不會 fallback 到另一邊。輪替先耐久撤銷載入權並將兩端寫入清理日誌，再寫入
  選定後端，最後才原子啟用新標記。刪除也先撤銷並記錄兩端，再清實體副本；
  因 Vault 鎖定等原因清不掉時，副本仍不可達，介面會顯示待清理警告並提供重試。
  系統認證儲存區可在重啟後直接沿用；加密保管庫重啟後必須先解鎖。刪除或備份還原後，
  已執行中的 Agent 仍可使用記憶體內的舊密碼直到本次停止或應用程式重啟，之後不再沿用。
  備份匯出刻意移除主機權威與清理日誌；還原一律不啟用舊主機密碼、記錄兩端待清理，
  並關閉下次使用已保存密碼。獨立常駐服務應放在只有服務帳號可讀的檔案，改用
