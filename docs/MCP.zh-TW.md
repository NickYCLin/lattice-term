...
CLI、Qwen Code、OpenCode、Copilot CLI 的終端工作階段，都會自動載入同一個
MCP adapter：Codex、Claude Code 與 Copilot 走程序參數，Gemini 與 Qwen 走
程序範圍的臨時設定檔，OpenCode 走一次性的內嵌設定，都不會動到所選帳號的
`config.toml`、`.claude.json`、`~/.gemini`、`~/.copilot/mcp-config.json` 或
OpenCode 的全域設定。因此在工作階段裡的 CLI
可以直接使用下方明確授權的 SSH／SFTP 與遠端工具，不必請你到終端機代打
指令；每次呼叫仍由該 CLI 自己先詢問，再經過桌面的授權檢查。自己帶了
`--mcp-config` 或 `--additional-mcp-config` 的啟動參數、用
`--safe-mode`／`--bare` 開的 Claude Code 或 Qwen、`--pure` 模式的 OpenCode，
...
SSH 授權另外可以開放「AI 臨時提出的指令」。這一項不預先核准任何內容：
client 用 `ssh_run_command` 送出指令原文後，桌面會跳出卡片顯示連線名稱、
提出的 client 與完整指令，等你按下允許才會開通道執行。拒絕、兩分鐘內沒
人回應、撤權或關掉桌面都代表沒有執行。指令限單行、最多 4096 bytes、不接
受控制字元（避免在卡片上看不出換行後還接了什麼），逾時固定 60 秒。權限
等同該 SSH 帳號，不是沙箱；同時最多 8 筆等待中的提案。

核准卡片另外提供「允許並在一段時間內不再問」（預設 15 分鐘，後端上限
120 分鐘）。這個免問期只屬於該筆授權、只存在記憶體，撤權、重連或關閉桌
面就消失，設定頁會顯示剩餘時間並可立刻恢復每次詢問。
...
2. 設定頁「MCP 遠端操作授權」選擇既有連線，選「只能看」或「完全開放」，按「開放給 MCP」。名稱直接用連線名稱。
   - 畫面（RDP／VNC／Lattice Remote）：只能看＝截圖；完全開放＝截圖加鍵盤滑鼠。Lattice Remote 主機若也分享了 Fleet 工作區，會同時開一筆 Fleet 授權（只能看＝狀態與輸出；完全開放＝再加派送、取消與啟動）。
   - SSH：只能看＝主機資訊；完全開放＝再加 AI 臨時提出的指令（每次仍要你在桌面同意）。
   - SFTP：只能看＝列目錄與下載；完全開放＝再加上傳。遠端目錄要自己填，本機目錄預設是「下載」資料夾。
   - 固定指令、透過 SSH 分享 Fleet 或只開其中幾項，展開「自訂細項」照舊逐項勾選並確認。
3. SSH 可開放 Linux 主機資訊與固定指令。指令由使用者事先填妥，AI 只選 `planId`，不能插入參數。另可單獨開放臨時指令，改成每次由使用者在桌面逐筆核准。SFTP 可開放目錄清單、上傳與下載，兩個方向分別授權。
...
輸入需 `targetId`、`snapshotId`、`frameId`、`action` 與 `requestId`。啟用 `input` 後，`capture_remote_screen` 才會提供 `snapshotId`。這個憑據綁定 client、授權與連線世代，十秒後失效；同 client 的新擷取會取代舊憑據。座標使用回覆的 `width`／`height` 原始像素（`coordinateSpace: framePixels`、`scale: 1`），不能使用視窗縮放後的 CSS 座標。

- `click`：`x`、`y`、`button`（0 左／1 中／2 右）。
- `move`：`x`、`y`。
- `drag`：起點 `x`／`y`、終點 `toX`／`toY` 與 `button`。
- `scroll`：`x`、`y`、`horizontal`、`units`（-8～8，不能為 0）。
- `keys`：最多八個不重複按鍵，依陣列順序按下、反向放開。支援 Control、Shift、Alt、Meta、Enter、Escape、Tab、Backspace、Delete、Insert、Home、End、PageUp、PageDown、四個 Arrow 方向、Space、F1～F12、小寫 a～z 與 0～9。
- `text`：最多 48 個可列印 Unicode 字元，不接受控制字元；換行請另用 Enter。

每個動作都是有界批次，按下與放開一起排入同一個輸入通道，不提供跨呼叫維持按住的按鍵。等候通道期間仍可能撤權，因此取得通道後才重新檢查授權、憑據、畫面尺寸與影像內容；畫面變動或超時回 `not_ready`，需重新看圖。比對的是整張畫面的粗略灰階縮圖（24×24 格）：閃爍的游標、跳動的時鐘或壓縮雜訊不算變動，換視窗、開對話框或捲動這類真正改變畫面的事則會被擋下；影格無法解碼時退回逐位元組比對。相同畫面內容的新 frame ID 不視為畫面變動。`capturedAt` 是桌面收到影格的時間，不是遠端應用程式的完成確認。

接受動作前會消耗此連線所有 client 的畫面憑據，避免多個 client 依同一張舊圖連續操作。相同 client／request ID 的相同重送沿用十五分鐘去重，不重按第二次；新 request ID 不能重用已消耗的憑據。`submitted: true` 只表示已送入通道，必須重新擷取並確認結果。
