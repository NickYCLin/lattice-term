...
可以直接使用你已經連上的 SSH／SFTP 與遠端連線，不必請你到終端機代打
...
你開著的連線就是 MCP 能碰到的範圍，不需要逐條授權。桌面每次處理 MCP
請求前會掃一次在線的工作階段，沒有授權的就自動補上：SSH 給主機資訊與
...
操作授權」只負責列出目前開放中的連線與範圍，並提供「暫停十分鐘」把某條
連線先收回；十分鐘後它會隨著連線重新開放。
...
1. 在桌面正常連線，確認主機金鑰與帳號；MCP 不代為確認、不使用其他已存帳號登入。
2. 連上之後就能用，不必再設定。設定頁只是讓你看目前開放了哪些連線與範圍。
3. 固定指令（`ssh_exec_job` 的 `planId`）不在自動範圍內，要用的話仍由使用者自行建立。臨時指令則每次都在桌面逐筆核准。
...
盲目重送。暫停、桌面失聯或 SSH 重連都會結束目前這筆授權；已接受的
