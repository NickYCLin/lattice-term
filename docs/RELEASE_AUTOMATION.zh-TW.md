...
1. 一般 push 與功能 PR 在 Linux amd64 執行前端檢查、Rust 格式、測試與 lint，並於 macOS 驗證遠端文字編輯的原生權限與加密連線；PR 另外檢查 Conventional Commits。符合路徑條件的功能 PR 亦會建立 Windows 測試安裝包、驗證 Windows 分享端拒絕文字編輯的邊界，以及執行既有 iOS 未簽章建置驗證；這些測試產物不是正式發布。
