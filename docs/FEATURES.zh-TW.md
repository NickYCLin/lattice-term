...
| 進階 Agent 與行動能力 | **部分完成** | 排程任務、接續執行與同時執行上限、提示佇列，Linux 的 bubblewrap 檔案範圍沙箱，跨程序背景服務（勾選「留在背景」的工作階段在關閉 LatticeTerm 後繼續執行、下次開啟自動接回），以及讓外部 AI 透過 MCP 唯讀查看分享的背景工作階段已完成；遠端 Fleet、macOS／Windows 的沙箱與 iOS 上架仍待完成。 |
...
- **MCP Server（唯讀）**：LatticeTerm 可當成 MCP 伺服器，讓 Claude Code、Codex、Gemini CLI 等外部 AI 工具唯讀查看你在 Agent Fleet 頁勾選「分享給 MCP」的背景工作階段：列出狀態與來源、以 cursor 增量讀取清理過的終端輸出、等待狀態改變。只有「留在背景」的工作階段能分享，預設不分享、可隨時撤銷；不能啟動、送指示或停止任何東西。設定片段在 Agent Fleet 頁可直接複製，細節見 [MCP Server](MCP.zh-TW.md)。
