...
- **MCP Server**：LatticeTerm 可當成 MCP 伺服器，讓 Claude Code、Codex、Gemini CLI 等外部 AI 工具查看你在 Agent Fleet 頁勾選「分享給 MCP」的背景工作階段：列出狀態與來源、以 cursor 增量讀取清理過的終端輸出、等待狀態改變。再勾「允許 MCP 送指示與停止」它才能對那個工作階段送指示（走同一套提示佇列）、清佇列或結束它；打開「允許 MCP 啟動已保存的背景啟動項目」它才能照你保存的項目啟動新的背景工作階段。三種授權都預設關閉、可隨時撤銷，每個工作階段旁會顯示最近是哪個 client 做了什麼。設定片段在 Agent Fleet 頁可直接複製，細節見 [MCP Server](MCP.zh-TW.md)。
  派送需要 CLI 官方就緒回報，會等待啟動指示並避開使用者編輯中的提示；撤權與清除 MCP 佇列會保留使用者自己的排隊工作。重開視窗載入實際啟動權限，設定保存同步到背景服務；request ID 防並行重送，慢 client 不會無限累積等待或輸出。沒有官方 hook 的 CLI 需手動操作；SSH／SFTP 與遠端畫面工具仍未實作。
