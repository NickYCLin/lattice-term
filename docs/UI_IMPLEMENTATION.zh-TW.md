...
對話側欄把本機 Agent 工作階段與對話放進同一棵樹：同一個資料夾底下可以同時有工作階段與對話，靠圖示與狀態點區分，沿用「工作階段」頁保存的資料夾、專案與排序。點選工作階段會切回原終端，不建立新的程序；工作階段列不提供改名與刪除，那些仍由「工作階段」頁負責。拖曳會寫回共用配置，沒在執行的工作階段其位置也會原封保留。對話列提供刪除入口，仍先確認；確認刪除後立即保存清單，避免關閉視窗時被舊的延遲保存內容還原。

語言選擇以原文名稱顯示九種語系，支援方向鍵選取。英文與繁中隨入口載入，其他語系按需載入；較新的未翻譯文案顯示英文。目錄沿用 #232 的翻譯，未宣稱完成各語系母語審校。
...
- 舊的 whole-payload 讀寫指令維持 32 MiB 上限，但編輯器不使用其無版本覆寫路徑；新的文字讀寫指令獨立限制 1 MiB，核對 revision 並保留原件。
