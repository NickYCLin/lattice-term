...
- **Claude、Gemini 與 Antigravity 一輪一個程序，Codex 一個對話一個 app-server**。Claude Code、Gemini 與 Google Antigravity 每則訊息以官方 headless JSON 模式啟動一次程序：`claude -p --output-format stream-json --verbose --include-partial-messages`、`gemini --output-format stream-json`、`agy --output-format stream-json`，提示從 stdin 送入並關閉。Codex 不論權限模式一律走 `codex app-server`（JSON-RPC over stdio），由 `agent_chat/codex_server.rs` 為每個 LatticeTerm 對話開一個伺服器：沒有伺服器時 spawn 程序、送 `initialize`／`initialized` 後 `thread/start`（有既有 thread ID 則 `thread/resume`），再送 `turn/start`。Codex 載入 thread 時會持有單一寫入者鎖，伺服器活著的期間 Codex Desktop 就無法開同一個對話，所以伺服器在沒有進行中的輪次、排隊的輪次或待確認的插話時閒置滿 1 分鐘，LatticeTerm 就關閉它的 stdin 讓它自行結束並釋放鎖（5 秒內沒結束才強制結束）。1 分鐘內繼續對話會沿用同一個伺服器，只送 `turn/start`；超過之後再送訊息則重新 `thread/resume`，啟動會慢一些。若對話正被 Codex Desktop 開著，LatticeTerm 會提示先到那邊關閉。同一對話一次只跑一輪；刪除對話（`agent_chat_close`）、程序退出或 LatticeTerm 關閉時也會清掉。`turn/start` 的 `approvalPolicy`、`sandboxPolicy`、`cwd` 與 `model` 每輪重送，所以權限與模型仍可逐輪調整。提示一律不放在命令列參數，因此不受參數長度限制也不會出現在程序清單。程序以使用者權限執行，工作目錄由使用者選擇並經 canonicalize 與 is_dir 驗證。
...
- **保存**：`localStorage` 的 `latticeterm.agentAutomations.v1`，最多 50 個排程、每個保留最近 20 次執行；會一起放進加密備份。還原舊版備份（當時還沒有這份資料）時保留這台電腦現有的排程，不會清空。指示是使用者明確寫下的任務內容，屬於刻意保存的設定，不是單次提示。
...
Fleet 的「新專案」、「加開 CLI」與對話頁均提供 Codex 的 CLIProxyAPI
選項。先自行啟動 CLIProxyAPI 並完成上游帳號登入，再到設定頁儲存代理
位址與 API 金鑰，即可選擇代理提供的模型，不必修改 Codex 設定或登入
Codex 的原生帳號。仍須安裝 Codex CLI，由它提供終端機與對話的工具執行。

設定頁可以加入多組代理（上限 8 組），每組有自己的名稱、位址與金鑰，
同一台伺服器也能用兩把金鑰分開登記。模型選單先選代理、再選該代理的
模型；代理之間不共用清單，選到哪一組就用哪一組的位址與金鑰。

桌面端以 `/healthz` 測試位址、讀 `/v1/models` 取得模型。清單讀不到時，
可自行填入模型 ID。探索請求不跟隨轉址，也不走系統代理，並設有回應
大小上限與逾時；模型推論則由 Codex 透過代理的 Responses API 執行。

代理金鑰存在系統金鑰圈或加密保管庫，各組代理分開存放並綁定自己的
位址；改位址後必須重新儲存相符的金鑰，移除代理時也會一併清掉金鑰。
啟動時由後端讀取，透過子程序環境變數提供，前端與儲存的啟動參數都不
含金鑰。LatticeTerm 為每次啟動產生專用 provider 名稱，並在名稱帶上代
理識別碼，避免 Codex 合併舊 provider 的認證與標頭；不覆寫使用者的
`config.toml` 或 `auth.json`。重開工作階段會依識別碼找回原本那組代理
的位址並重新取得金鑰，找不到就直接拒絕啟動，不會改用別組。
未設定 API 金鑰的代理也能使用；保管庫無法讀取或金鑰綁定不符時會停止
啟動，不會改用原生帳號。背景服務使用自己的憑證存放環境。

終端機工作階段裡的 `/model` 也改列代理的模型。啟動時後端向該組代理
讀一次 `/v1/models`，寫成只屬於這個程序的暫存 `model_catalog_json`，
工作階段結束就刪除。每個條目都照 Codex 對「不認識的模型」所用的設定
產生，包括它內建的提示（`vendor/openai-codex-prompt`）與同一套工具，
送到代理的請求和沒有這份清單時相同，只是選單改成代理的模型。
`/model` 的選擇會寫進 `$CODEX_HOME/latticeterm-cliproxyapi*.config.toml`
這個專用 profile，不會把代理的模型 ID 寫成原生 Codex 的預設模型。

這需要 Codex 0.134.0 以後的分層 `--profile`；版本較舊、代理讀不到或
逾時（5 秒）時照舊啟動，`/model` 仍是 Codex 原生清單，而實際使用的
模型依然是啟動時選的那一個。對話頁有自己的模型選單，不受影響。
Windows 上帶 `--profile` 的 Codex 不在 MCP 輸入驗證支援範圍，這類代理
工作階段需手動輸入。

切換原生模型與代理會清除原生對話 ID，保留畫面和交接摘要。對話背景
程序也會核對代理位址與金鑰是否變更，避免沿用舊連線。
...
- npm 型的安裝指令找不到 `npm` 時，`install.requirement` 會帶出 Node.js 前置安裝：Windows 上以 PowerShell 讀取 `nodejs.org/dist/index.json` 取最新 LTS 的 MSI，下載後與同版 `SHASUMS256.txt` 比對 SHA-256，吻合才以 `msiexec /passive` 安裝，不符就刪檔中止；其他平台只提供 nodejs.org 下載頁。偵測 `npm` 時除了 PATH 也會看 Program Files／LocalAppData 的 `nodejs` 資料夾，所以裝完 Node.js 不必重開 LatticeTerm 就能接著安裝 CLI。
- CLI 預設以啟動 LatticeTerm 的使用者權限執行。Linux 上若裝有 bubblewrap，啟動表單可勾選「沙箱：只能改工作目錄」：`sandbox_arguments` 產生 `bwrap --ro-bind / / --dev /dev --proc /proc --bind /tmp /tmp --bind <工作目錄> … --unshare-pid --die-with-parent --chdir <工作目錄> --` 的參數，只把工作目錄、該 CLI 已存在的登入／狀態目錄（例如 `~/.claude`、`~/.claude.json`、`~/.codex`、`~/.cache`、`~/.npm`）與帳號設定檔目錄重新綁定為可寫；網路維持共用，CLI 才連得到模型。`sandbox_tool()` 不只找二進位，還會用 `/bin/true` 探測一次 bwrap 真的能建立 user namespace——Ubuntu 24.04 起預設 `kernel.apparmor_restrict_unprivileged_userns=1`，沒有為 bwrap 啟用 AppArmor 設定檔時會在 `setting up uid map` 失敗；探測失敗就不提供選項，`runtime_summary` 的 `agentSandboxAvailable` 決定表單是否顯示。啟動時仍拒絕而不是靜默不隔離。這是檔案範圍策略，不是完整容器：CLI 仍看得到唯讀的整個檔案系統與網路。macOS 上 `sandbox_tool()` 改為探測 `/usr/bin/sandbox-exec`，`seatbelt_arguments` 產生 `(allow default)(deny file-write*)` 再放行工作目錄、狀態目錄、`/private/tmp`、`/private/var/folders`、`~/Library/Caches` 與 `~/Library/Keychains` 的規則，最後以後出現者優先的規則再次拒絕 hooks／MCP 設定檔；所有路徑先 canonicalize，再用 `-D W0=…`／`-D R0=…` 參數帶入，政策檔無法解析時拒絕啟動。 啟動前會先把該 CLI 尚不存在的狀態目錄（以及 Claude Code 的 `.claude.json`，內容為空物件）建好，因為 bwrap 只能綁定已存在的路徑，否則第一次登入會寫不進去；既有檔案一律不動。
...
| 工具專用語意 Adapter | 部分完成 | Codex `notify`、Claude Code、Gemini CLI、Hermes Agent、Qwen Code lifecycle hooks，以及 OpenCode、GitHub Copilot CLI plugin events 已接上 Reporter；Aider 以官方 `--notifications-command` 回報「需要注意」（它在一般輸入與 y/n 確認前都會觸發，無法區分，所以不回報「完成」、不放行佇列；使用者自己設過通知就不加）；Hermes 已提供 token buckets，舊工作區續接 recipe 與保守的 session ID 擷取仍保留。Factory Droid、Cursor Agent 與 Grok Build 的 hooks 只從使用者或專案的設定檔讀取，Kimi 的 `--config-file` 會整份取代使用者設定，四者都沒有只影響單一程序的注入方式，為了不改寫使用者檔案暫不整合。Claude Code 與 Codex 在官方回報完成後，會從它們自己的本機紀錄檔（依已擷取的原生 session ID 精確定位，使用帳號設定檔時略過）重算累計 token：Claude 依回覆 message id 去重後加總，Codex 取最後一筆 `token_count`；cost 仍未擷取 |
...
| 遠端 Agent Fleet | SSH 與 Relay MCP 已實作，Relay 尚待外部主機驗收 | 核准工作區內的多 PTY 可透過 MCP 分別列出、讀取、啟動與控制；Fleet 頁的「遠端 Agent Fleet」區塊用同一套授權並排顯示遠端工作階段的文字輸出、排入提示、取消與啟動；巢狀派工尚未提供 |
...
| 任務編排 | 部分完成 | broadcast prompt 與每個工作階段的提示佇列已完成；佇列上限 16 則，只有官方整合回報 `Done`／`Idle` 才放行一則，heuristic 猜測不放行；對話模式的排程任務與「接在某個排程之後」的依賴鏈已完成（見下）；Fleet 工作階段也能指定「等待」另一個工作階段，對方由官方整合回報結束才放行，並可設定同時工作上限，超過時佇列先等、不中斷已在跑的工作；CPU／記憶體等資源限制仍待實作 |
| 權限隔離 | 部分完成 | Linux 上可勾選以 bubblewrap 啟動：整個檔案系統唯讀，只有工作目錄、該 CLI 自己的登入／狀態目錄、帳號設定檔目錄與 /tmp 可寫，PID 命名空間隔離、網路共用；選項與工作區項目一起保存。macOS 以系統內建的 `sandbox-exec`（Seatbelt）做同樣的寫入限制（沒有 PID 隔離）；Windows 尚無不動到使用者檔案權限的對應機制，也還沒有網路或資源限制 |
...
SSH 路徑已透過 `remote_fleet` 接入：本機使用者核准一條已信任的 SSH
連線、遠端執行檔與工作區，工具以專用 channel 連到遠端 scoped MCP
...
安全啟動工作區、工作區命名／排序、明確確認的批次重新啟動、broadcast prompt 與每個工作階段的提示佇列已完成。佇列刻意只由官方整合事件放行：靜默 watchdog 產生的 heuristic `Idle` 只是猜測，據以送出提示會打斷一個其實還在跑的 CLI。每次回報結束只放行一則，佇列上限 16 則，工作階段結束時連同 PTY 一起丟棄。排程與任務依賴已在對話模式實作（`agentAutomations.ts` 的每日／間隔／接續排程與依賴鏈）；Fleet 工作階段也有跨工作階段的等待關係與同時工作上限：只有會正式回報完成的 CLI 能被等，互等或等自己會被擋下；被等的工作階段結束時，後面的照常放行。等待關係只在同一個 registry 內成立，視窗內的工作階段與背景服務的工作階段不能互等；上限由桌面保存，背景服務每次連上時同步。下一步是 CPU／記憶體等資源限制。預設只保存狀態 metadata，不保存提示或終端逐字稿；任何錄製都必須由使用者明確開啟並設定保存位置。
