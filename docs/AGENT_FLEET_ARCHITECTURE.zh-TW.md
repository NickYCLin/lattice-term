...
| MCP Server | 已完成（A＋B 階段） | `lattice-term mcp` 以 stdio 提供觀測（`get_capabilities`／`list_agent_sessions`／`read_agent_output`／`wait_agent_state`）與受控協作（`list_launch_plans`／`launch_agent`／`send_agent_prompt`／`cancel_agent_task`），以 observer 角色連 daemon。分享、可控、啟動是三個分開的授權，都在介面勾選、預設關閉；request ID 去重 15 分鐘；沒有「中止本輪」。SSH／SFTP、遠端畫面（#180 的 C～D）未實作，見 [MCP Server](MCP.zh-TW.md) |
...
- **觀察者角色（MCP）**：`hello` 可帶 `role`（省略視為 `desktop`）與 `client`（MCP client 名稱）。`observer` 能 `sessions`（只回已分享的，附 `mcpControl`）、`observe`（依 cursor 讀某個已分享工作階段的保留輸出，`AgentOutputRange` 帶 `startOffset`／`endOffset`／`nextCursor`／`truncated`）、`plans`；對桌面以 `controlSet` 授權的工作階段還能 `prompt`（`queue` 走 `agent::enqueue`，`now` 走 `agent::send` 且 working／needsAttention 時拒絕）與 `cancel`（`queue` 清佇列、`session` disconnect）；桌面以 `mcpPlansReplace` 交付允許的啟動項目（已由 `launch_request_from_plan` 準備好）後能 `launchPlan`（強制 detached，成功後自動分享＋可控並向桌面廣播 `launched` 事件）。其他請求一律拒絕；事件只轉送已分享工作階段的 `state`／`closed`／`model`／`usage`／`queue`，永遠不送 `data`；不計入 `client_count`，所以不影響無視窗排程，但啟動開關開著時 daemon 不會閒置結束。分享集合、授權、最近操作與 15 分鐘的 request ID 結果都放在 daemon 記憶體，工作階段結束自動移除。`lattice-term mcp --data-dir <dir>` 就是用這個角色的 stdio JSON-RPC adapter（`agent_daemon/mcp.rs`），細節見 [MCP Server](MCP.zh-TW.md)。
