...
| MCP Server（唯讀觀測） | 已完成（A 階段） | `lattice-term mcp` 以 stdio 提供 `get_capabilities`／`list_agent_sessions`／`read_agent_output`／`wait_agent_state`，以 observer 角色連 daemon，只看得到使用者在介面勾選「分享給 MCP」的背景工作階段；不能啟動、送指示或停止。受控協作、SSH／SFTP、遠端畫面（#180 的 B～D）未實作，見 [MCP Server](MCP.zh-TW.md) |
...
- **觀察者角色（MCP）**：`hello` 可帶 `role`（省略視為 `desktop`）。`observer` 只能 `sessions`（只回已分享的）與 `observe`（依 cursor 讀某個已分享工作階段的保留輸出，`AgentOutputRange` 帶 `startOffset`／`endOffset`／`nextCursor`／`truncated`），其他請求一律拒絕；事件只轉送已分享工作階段的 `state`／`closed`／`model`／`usage`／`queue`，永遠不送 `data`；不計入 `client_count`，所以不影響無視窗排程與閒置結束。分享集合放在 daemon 記憶體（`shareSet`／`shared` 由桌面操作），工作階段結束自動移除。`lattice-term mcp --data-dir <dir>` 就是用這個角色的 stdio JSON-RPC adapter（`agent_daemon/mcp.rs`），細節見 [MCP Server](MCP.zh-TW.md)。
