...
代理金鑰存在系統金鑰圈或加密保管庫，綁定位址；改位址後必須重新儲存
相符的金鑰。啟動時由後端讀取，透過子程序環境變數提供，前端與儲存的
啟動參數都不含金鑰。LatticeTerm 為每次啟動產生專用 provider 名稱，
避免 Codex 合併舊 provider 的認證與標頭；不覆寫使用者的 `config.toml`
或 `auth.json`。重開工作階段仍會保留代理位址並重新取得金鑰。
