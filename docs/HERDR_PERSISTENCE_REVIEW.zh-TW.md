...
可能含本機路徑與啟動設定，不應貼到公開 issue；目前沒有自動匯回或復原介面。
先前已被舊版覆蓋、又沒有備份的內容，也不能由這次修正憑空還原。

## 後續優先順序

1. **專案獨立保存**：建立明確的 project 記錄、刪除操作和失敗狀態，
   即使所有 CLI 已退出且沒有原生對話 ID，專案仍存在。
2. **原生端單一寫入者**：保存專案、session intent 與還原結果，
   前端只顯示狀態，減少多視窗及關機時的保存競爭。
3. **可操作的復原入口**：顯示未還原原因、逐項重試，以及備份預覽／匯回，
   不要只靜默保留資料。
4. **故障驗收矩陣**：實際測試正常關閉、強制終止、電腦重開機、
