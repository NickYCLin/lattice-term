...
工作區保存不再以 CLI 的結束碼或是否取得原生對話 ID 決定是否留下分頁。
正常退出、異常退出及尚未取得對話 ID 的分頁，都會保留啟動資訊；
使用者明確關閉分頁後，才從下一份工作區快照移除。
這補上程序先結束、介面隨後保存時可能遺失項目的路徑，
不代表已完成實機重開機或斷電耐久性驗收。

「外部對話」可選擇在啟動時自動開啟全部本機 Codex／Claude 對話。
選項預設關閉；開啟時先保存全部啟動資訊，再逐項接續。
已接入的項目依助理、帳號及原生對話 ID 去重。
批次讀取不受預覽清單 100 筆限制，但超過 1024 筆或掃描上限時會明確失敗，
不會只取前幾筆卻顯示完成。工作區快照上限同步為 1024 筆；
同時執行的程序仍受原有後端上限約束，尚未啟動的項目留在復原區。
匯入的原生對話接續失敗時，不會以新對話取代原本的對話。

在對話頁選擇既有 CLI 工作階段，現在會留在對話頁。
Codex／Claude 的訊息依該工作階段確切的原生對話 ID 及帳號讀取，
不會猜測同資料夾的最新對話。訊息送入原本工作階段的佇列，
不另外啟動 headless CLI；終端機與聊天介面操作同一個程序。
紀錄每兩秒在前次讀取完成後更新，顯示以 CLI 已寫入的紀錄為準；
權限核准及尚未支援的互動仍需回到終端機。
