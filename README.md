...
<table>
  ...
  <tr>
    ...
    <td width="50%" valign="top">
      <h3>排程與背景工作</h3>
      <p>為固定任務設定排程，回來查看執行結果。Agent 工作階段可留在本機背景服務，關閉視窗後繼續執行，下次開啟再接回；MCP 第一次分享只開放工作階段資訊與狀態，讀取內容、送指示與啟動保存項目需分開授權。桌面開啟時，可逐項授權既有 SSH／SFTP 連線的指定指令、主機資訊與檔案操作。另可獨立授權遠端畫面與鍵鼠，或透過 SSH 操作核准工作區內的多個 Agent 背景 PTY。最近 256 筆操作紀錄可安全保存在此裝置，跨背景服務重啟還原。</p>
    </td>
  </tr>
</table>
