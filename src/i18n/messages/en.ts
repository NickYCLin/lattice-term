...
export const en: Messages = {
  ...
  "agents.mcp.hint": "LatticeTerm can act as an MCP server so another AI tool (Claude Code, Codex, Gemini CLI, …) can see the background sessions you choose to share: list their state, read terminal output, wait for a state change. Only sessions kept in the background and ticked \"Share with MCP\" below are visible; tick \"Let MCP send prompts and stop it\" as well before it may prompt, clear the queue or end that session, and turn on the launch switch below before it may start new sessions. Add one of the snippets below to your AI tool to connect.",
  ...
  "agents.mcp.launch.hint": "An external AI can call launch_agent to start the saved plans marked \"keep in the background\" ({count} right now), exactly as you saved them (CLI, arguments, working directory); a session it starts is shared and controllable. While on, the background service stays running.",
